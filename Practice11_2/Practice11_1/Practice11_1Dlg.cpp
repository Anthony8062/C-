...
BOOL CPractice11_1Dlg::OnInitDialog()
{
	...
	//현재날짜를 불러와서 Combo Box와 Edit Box에 표시하여준다.
...
void CPractice11_1Dlg::OnPaint()
{
	...
	{
		//날짜를 저장할 변수 지정한 뒤 모두 0으로 초기화 한다.
		int m_nDays[6][7] = {0,};
		int i, j, x, y;

		x = 30;
		y = 105;	//대화상자와 컨트롤의 배치에 따라 변경될수있다.
		...
		//DLL 로 제작한 함수를 사용한다.
		//Calender(m_nYear, m_nMonth, &m_nDays[0][0]); //임시적 Dll 링킹 호출

		HINSTANCE hDll;
		hDll = LoadLibrary(_T("CalenderDll.dll"));//dll 파일을 읽어온다.

		//dll 에서 읽어올 함수 형을 지정한다.
		...
		//날짜를 표시하여 준다. 단 날짜 데이터가 0일 경우는 표시하지 않는다.
		for (i = 0; i < 6; i++)
		{
			x = 30;
			for (j = 0; j < 7; j++)
